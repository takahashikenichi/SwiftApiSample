...
class YahooLocationWrapper {
    ...
    let detailParam = "&detail=full"   // 出力に詳細情報を含める
    ...
    static var sharedInstance = YahooLocationWrapper()
    
    func requestList(queryWord: String, location: CLLocationCoordinate2D,
        distance: CLLocationDistance,
        completion: ((response: NSURLResponse!, data: [ShopModel]?, error: NSError?) -> Void)) {
            ...
            NSURLConnection.sendAsynchronousRequest(req, queue: queue,
                completionHandler: {
                    ...
                    if error == nil {
                        ...
                        jsonResult = NSJSONSerialization.JSONObjectWithData(
                            data,
                            options: NSJSONReadingOptions.AllowFragments,
                            error: &resultError) as? NSDictionary
    ...
    func parseResponseJson(json: NSDictionary) -> [ShopModel]? {
        ...
        for feature in features! {
            ...
            shopData.thumbnailUrl = property!["LeadImage"] as? String
            ...
            if let geo = geometry {
                ...
                shopData.location = CLLocationCoordinate2DMake(
                    atof(coordinatesArray[1]) as CLLocationDegrees,
                    atof(coordinatesArray[0]) as CLLocationDegrees)
            }
